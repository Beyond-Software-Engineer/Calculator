...
class Student:
    ...
    def returned_message(self):
        ...
        bonus = 0
        message = self.get_rental_list()
        total_amount = self.get_total_fine()
        total_amount = self.discounted_bonus(total_amount)
    ...
    def get_total_fine(self):
        total_fine: float = 0
        for rental in self.rentals:
            total_fine += rental.get_fine()

        return total_fine

    def discounted_bonus(self,total_amount:float) -> float:
        while self.bonus > 7 and total_amount > 1:
            self.add_bonus(-7)
            total_amount -= 1
